...
- **Per-app task semantics** — UE = photo-challenge per waypoint/neighborhood; Roadtripper = road-trip prompts (pit stops, scenic drives, local-delicacy stops). Task generation runs per-app via the `--app` flag; per-consumer differentiation lives on individual task docs (no separate `tasks_rt`/`tasks_ue` collections).
