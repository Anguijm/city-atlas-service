...
- **Cross-consumer contract** — `src/schemas/cityAtlas.ts` is the contract between this repo and both UE + Roadtripper. Consumers import via copy or git URL (no published npm package). Treat the Zod schemas (City, Neighborhood, Waypoint, Task, SeasonalVariant) as a public API; breaking changes require coordinated consumer deploys.
...
- **Firestore topology** — Write-once, read-many from two consumers. Pipeline writes to `cities/{cityId}` + nested `neighborhoods/`, `waypoints/`, `tasks/` subcollections, plus flat denormalized copies in `vibe_neighborhoods`, `vibe_waypoints`, `vibe_tasks`, plus `seasonal_variants`, `pending_research`, `health_metrics`. Never write `saved_hunts/*` or `cache_locks/*` (app-owned). No `tasks_rt`/`tasks_ue` collections; per-consumer task differentiation lives on individual task docs.
- **Per-app task generation** — `--app {ue|roadtripper}` flag selects the task-template config. Adding a third consumer should be a config change, not a code change.
